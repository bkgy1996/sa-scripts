function [y_predict,performance,pmasks,all_masks] = cpm_main(x,y,varargin)
...
addParameter(p,'pmatrix',defaultkfolds,@isnumeric);
addParameter(p,'lastmdl',defaultkfolds,@isnumeric);

parse(p,x,y,varargin{:});

pthresh = p.Results.pthresh;
kfolds = p.Results.kfolds;
lastmasks = p.Results.pmatrix;
lastmdl = p.Results.lastmdl;
...
[y_predict,pmasks,all_masks]=cpm_cv(x,y,pthresh,kfolds,lastmasks);
